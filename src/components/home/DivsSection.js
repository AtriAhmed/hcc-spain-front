...
const DATA = [
  {
    ...
    content: `Obtaining Halal certification through Halal Correct Germany Certification ensures that
  ...
  },
  {
    ...
    content: `Our certification process is comprehensive and thorough, ensuring that your
    manufacturing and supply chain practices align with halal requirements.
    We conduct audits, inspections, and reviews to verify compliance, providing you
    with the necessary documentation to demonstrate your commitment
    to halal integrity.`
  },
  {
    title: "Market Advantage",
    content: `By obtaining Halal Correct Germany Certification, you gain a significant advantage in the
  ...
  return (
    <div className='grid grid-cols-12 max-w-6xl mx-auto md:py-10 gap-4'>
      ...
      <section className="col-span-12 md:col-span-7 flex flex-col relative h-full">
        <div>
          {DATA.map((data, index) => (
            <HoverDiv key={index} data={data} index={index} handleHover={handleHover} />
          ))}
        </div>
        <div
          className={`absolute left-0 top-0 border-l-2 border-black h-[33%] hidden md:block`}
          style={{ transform: `translate(0px,${(hoveredIndex * 100)}%)`, transition: "transform 0.5s ease" }}
        ></div>
      </section>
    </div>
  );
