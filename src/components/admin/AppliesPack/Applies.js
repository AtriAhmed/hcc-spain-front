...
export default function Applies() {
    ...
    function getItems(){
        axios.get('/api/get-applies').then(res=>{
                setItemsList(res.data.applies);
            setLoading(false);
        }).catch(err=>{
    ...
    const deleteItem = (e,item) =>{
        ...
        Swal.fire({
            title: 'Delete Apply',
            text: `Are you sure to delete ${item.name} apply ?`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`/api/delete-apply/${item.id}`).then(res=>{
...
if(itemsList.length > 0){
        ...
        <div className='pb-3 hidden md:block col-span-3'>Explanation</div>
...
{        itemsList.map((item)=>{
            return (
                <div key={item.id} className="mx-0 grid grid-cols-12 text-center break-all">
                    ...
                    <div className='pt-3 hidden md:block col-span-3 truncate'>{item.explanation}</div>
                    ...
                    <div className='pt-3 col-span-3 md:col-span-2 pb-3 text-end sm:text-center'>
                        ...
                        <div className='col-span-12 sm:col-span-6 text-end sm:text-center'><button type='button' className='btn p-0' onClick={(e)=>{deleteItem(e,item);}} disabled={user.role_as == 1 ? false : true}><TrashIcon className={classNames(user.role_as == 1 ? "text-red-600" : "text-gray-400" , "block h-8 w-8")} aria-hidden="true"/></button></div>
                        </div> 
                    </div>
                </div>
            )
...
<InboxIcon className="block h-20 w-20" aria-hidden="true"/>
            <h3 className='text-2xl font-bold'>Theres no Applies</h3>
    ...
    <div className="max-w-7xl p-5 mx-auto">
        <div className='rounded-lg shadow-lg'>
            <div className="bg-gray-100 p-4 rounded-t-lg">
                <h5 className='mb-3 mb-0'>Applies ( {itemsList.length} )</h5>
            </div>
            <div className="p-5">
        ...
        <ViewApplyModal show={viewModalShow} hide={()=>{setViewModalShow(false);getItems()}} toview={toView} />
    </div>
  )
}
