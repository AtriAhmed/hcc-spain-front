...
const ContentItem = SortableElement(({ items, setItems, handleItemContentChange, item, idx, language }) => {
  ...
  const deleteItem = (item, index) => {
    if (item.id)
      ...
      const updatedContent = items.filter((item) => items.indexOf(item) != index);
    ...
    <div className="grid grid-cols-12 w-full pl-4">
      ...
      {item.type === "title" ? (
        ...
        <div className="col-span-10 flex gap-2 p-2">
          {item.type === "image" && (item.file_path ? <img src={item?.file_path.includes("upload") ? `${process.env.GATSBY_API_URL}${item.file_path}` : item.file_path} alt="Preview" className="w-32 h-32 object-cover" /> : <Photo fontSize="inherit" className="text-slate-400 !text-[8rem]" />)}
