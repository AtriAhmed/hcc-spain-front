...
export default function Question() {
    ...
    function getQuestions(){
        axios.get('/api/get-questions').then(res=>{
                setItemsList(res.data.questions);
                setLoading(false);
        })
    }
    useEffect(()=>{
    ...
    const deleteItem = (e,item) =>{
        ...
        Swal.fire({
            ...
            text: `Are you sure to delete ${item.name} question ?`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`/api/delete-question/${item.id}`).then(res=>{
...
if(itemsList.length > 0){
        ...
        <div className='pb-3 hidden md:block col-span-3'>Question</div>
...
{        itemsList.map((item)=>{
            return (
                <div key={item.id} className="mx-0 grid grid-cols-12 text-center break-all">
                    ...
                    <div className='pt-3 hidden md:block col-span-3 truncate'>{item.question}</div>
...
<InboxIcon className="block h-20 w-20" aria-hidden="true"/>
            <h3 className='text-2xl font-bold'>Theres no Questions</h3>
    ...
    <div className="max-w-7xl p-5 mx-auto">
        <div className='rounded-lg shadow-lg'>
            <div className="bg-gray-100 p-4 rounded-t-lg">
                <h5 className='mb-3 mb-0'>Questions ( {itemsList.length} )</h5>
            </div>
            <div className="p-5">
...
<ViewQuestionModal show={viewModalShow} hide={()=>{setViewModalShow(false);getQuestions()}} toview={toView} />
    </div>
  )
}
